...
const overlayRoot = () => document.querySelector('.container');
function showOverlay(){
  const root = overlayRoot();
  root.classList.remove('is-hidden','exit');
  // trigger enter
  root.classList.add('enter');
  setTimeout(()=> root.classList.remove('enter'), 380);
}
function hideOverlay(){
  const root = overlayRoot();
  root.classList.add('exit');
  setTimeout(()=> {
    root.classList.remove('enter','exit');
